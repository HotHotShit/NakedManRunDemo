...
namespace Puppet3D
{
	public class ColladaExporter : IDisposable
	{
		...
		public XmlNode AddController(string id, Mesh sourceMesh, SkinnedMeshRenderer smr)
		{
			...
			string[] weightsArray = new string[sourceMesh.boneWeights.Length];
			...
			for (int i = 0; i < smr.sharedMesh.bindposes.Length; i++)
			{
				...
				Matrix4x4 mat = smr.sharedMesh.bindposes[i];
				/*if (parent != null)
				{
			...
			for (int i = 0; i < weightsArray.Length; i++)
			{
				...
				boneAssignedArray[i] = sourceMesh.boneWeights[i].boneIndex0 + " " + (2 * i).ToString() + " " + sourceMesh.boneWeights[i].boneIndex0 + " " + ((2 * i) + 1).ToString();
			}
