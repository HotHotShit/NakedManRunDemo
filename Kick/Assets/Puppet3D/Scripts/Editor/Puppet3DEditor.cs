...
namespace Puppet3D
{
	public class Puppet3DEditor : EditorWindow
	{
		...
		public static int _triangulationIndex, _numberBonesToSkinToIndex = 1, _skinningType;
		...
		void OnGUI()
		{
			...
			if (currentGUIChoice == GUIChoice.RigginSetup)
			{
				...
				if (GUI.Button(new Rect(80, 190 + offsetControls, 150, 30), "Set Driver"))
				{
				...
				if (DrivenKeyActive != null)
				{
					GUI.Label(new Rect(80, 225 + offsetControls, 150, 30), DrivenKeyActive.gameObject.name);
					if (GUI.Button(new Rect(80, 250 + offsetControls, 150, 30), "Set Driven"))
					{

						DrivenKeyActive.DriverColection[0] = new DrivenObject(Selection.gameObjects);
					}
					if (GUI.Button(new Rect(80, 280 + offsetControls, 150, 30), "Set End Position"))
					{
			...
			if (currentGUIChoice == GUIChoice.Skinning)
			{
				...
				KeepVoxels = GUI.Toggle(new Rect(80, 160 + offsetControls, 256, 20), KeepVoxels, " Keep Voxels");
				GUILayout.Label(" Voxel Res", EditorStyles.boldLabel);

				VoxelScale = EditorGUI.Slider(new Rect(80, 180 + offsetControls, 150, 20), VoxelScale, 1f, VoxelScaleMax);


				GUILayout.Label(" Blur Iter", EditorStyles.boldLabel);

				BlurIter = EditorGUI.IntSlider(new Rect(80, 200 + offsetControls, 150, 20), BlurIter, 0, 20);
		...
		static void RefreshMeshColors()
		{
			...
			if (BlackAndWhiteWeights && boneIndex > -1)
			{
				for (int i = 0; i < vertices.Length; i++)
				{
					...
					BoneWeight bw = currentSelectionMesh.boneWeights[i];

					if (bw.boneIndex0 == boneIndex)
					{
						float col = bw.weight0;
						colrs[i] = new Color(col, col, col);
					}
					else if (bw.boneIndex1 == boneIndex)
					{
						float col = bw.weight1;
						colrs[i] = new Color(col, col, col);
					}
